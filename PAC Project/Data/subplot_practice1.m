...
if AR_highWing == 1
    ...
    % 10/26/2021
        % Okay, this is the first time I have looked at this code in over a
        % year. I remember that the last idea I had was to modify the code
        % to draw a spline instead of just 5-6 straight lines. Now I have
        % learned about some various functions that perform similar
        % actions as a spline, namely pchip and makima. So I'm going to
        % modify the following code to draw these lines instead. I think
        % I'm going to try pchip first.
        
        % So the process I am going to follow is this, we know the end
        % point of the linear portion of the curve, and we know the stall
        % point. With these two points we can draw a spline(pchip) between
        % these two points. This constitutes the first nonlinear curve.
        
        % For the second nonlinear curve, I am going to determine the slope
        % of that spline(pchip) and then using the slope and the point just
        % above the linear end point on the first nonlinear curve, will
        % determine the end point for the second nonlinear curve.
        % Basically, just see where the first nonlinear curve begins, and
        % end the second nonlinear curve at the height, but at a negative
        % slope instead of a positive slope.
        

        % Array for end of linear portion of Cla curve
        aStarPoint = [aStarA_wing CLa_wing*(aStarA_wing - aL_0_wing)];
        
        % Array for top of Cla curve
        maxPoint = [alphaCLmax CLmax_wing];
        
        % First section (leading) increment array along x-axis 
        leadingIncrement = aStarPoint(1):.2:maxPoint(1);
        
        % Leading nonlinear curve
        leadingNonlinearCurve = makima([aStarPoint(1) maxPoint(1)],[aStarPoint(2) maxPoint(2)],leadingIncrement);
        
        % Slope of leading nonlinear curve (LNC)
        slopeLNC = (maxPoint(2) - aStarPoint(2)) / (maxPoint(1) - aStarPoint(1));
        
        % Final x-axis point on the last section (trailing) of the nonlinear curve
        stallPointX = ((maxPoint(2)-aStarPoint(2)) / (-slopeLNC)) + leadingIncrement(ceil(length(leadingNonlinearCurve)/1.4)) + alphaCLmax;
        
        % Final y-axis point on the last section (trailing) of the nonlinear curve
        stallPointY = leadingNonlinearCurve(ceil(length(leadingNonlinearCurve)/1.4));
        
        % Array of final points
        stallPoint  = [stallPointX stallPointY];
        
        % Last section (trailing) increment array along x-axis
        trailingIncrement = maxPoint(1):.1:stallPoint(1);
        
        % Array of nonlinear curve points
        nonlinearCurveArrayX = [aStarPoint(1) maxPoint(1) stallPoint(1)];
        nonlinearCurveArrayY = [aStarPoint(2) maxPoint(2) stallPoint(2)];
        nonlinearCurveIncrement = nonlinearCurveArrayX(1):.1:nonlinearCurveArrayX(3);
        
        % Nonlinear curve of CLa plot
        nonlinearCurve = makima(nonlinearCurveArrayX, nonlinearCurveArrayY,nonlinearCurveIncrement);
        
        % Airfoil Curve
        line(ax,alphaArray_airfoil,ClaArray_airfoil,'linewidth',2,'color','b');
    
        % Wing Curve
        alphaArray_wing_linear = [aL_0_wing; aStarA_wing;];
        ClaArray_wing_linear   = [0; CLa_wing*(aStarA_wing-aL_0_wing);];
        line(ax,alphaArray_wing_linear,ClaArray_wing_linear,'linewidth',2,'color','black');
        line(ax,nonlinearCurveIncrement,nonlinearCurve,'linewidth',2,'color','black');
        ...
        ylim([0 2]);
        grid on   
        legend('Airfoil','Wing','fontsize',12);
end
...
%         CLa_wing     = (CLa_wing_rad*pi)/180;
        ...
        % Final x-axis point on the last section (trailing) of the nonlinear curve
        stallPointX = ((maxPoint(2)-aStarPoint(2)) / (-slopeLNC)) + leadingIncrement(ceil(length(leadingNonlinearCurve)/1.2)) + alphaCLmax;
        
        % Final y-axis point on the last section (trailing) of the nonlinear curve
        stallPointY = leadingNonlinearCurve(ceil(length(leadingNonlinearCurve)/1.2));
...
% Nicolai
deltaAlphaZeroLift_rad = -dCl_dDeltaF*(1/Cla_airfoil_rad)*(flapAngleRad)*Kprimef;
deltaAlphaZeroLift_deg = (deltaAlphaZeroLift_rad*180)/pi();
...
aStarA_wing_Flaps = aStarA_wing;
    ...
    % Determination of Delta Clmax from airfoil curve
    Clmax_airfoil = max(ClaArray_airfoil);
    
    % Location of Clmax on Cla curve
    findClmax_airfoil = find(ClaArray_airfoil == max(ClaArray_airfoil));
    
    % Alpha at Clmax airfoil
    alphaClmax_airfoil = alphaArray_airfoil(findClmax_airfoil);
    
    % Alpha at Clmax airfoil with flaps
    alphaClmax_airfoil_flaps = alphaClmax_airfoil + deltaAlphaStall;
    
    % Airfoil Curve
%     line(ax,alphaArray_airfoil,ClaArray_airfoil,'linewidth',2,'color','b');

%     line(ax,alphaArray_airfoil_flaps,ClaArray_airfoil_flaps,'linewidth',2,'color','red');
%     line(ax,alphaArray_airfoil_flaps(1:8),ClaArray_airfoil_flaps(1:8),'linewidth',2,'color','red');
    ...
    alphaArray_airfoilLine = [alphaClmax_airfoil_flaps alphaClmax_airfoil_flaps];
    clArray_airfoil    = [0 4];
    line(ax,alphaArray_airfoilLine,clArray_airfoil,'linewidth',2,'color','black');
    
    
    
    
%     maxPointFlaps = [alphaClmax_airfoil_flaps CLmax_wing + deltaClmaxFlaps];
    
testValue = .1;
difference = 1;
%% Airfoil Curve with Flaps
while abs(difference) > .001
    % Okay need to transcribe aStarA point on airfoil curve with no flaps, to
    % airfoil curve with flaps

    findAStarACl = find(alphaArray_airfoil == aStarA_airfoil);
    
    clatAStarA   = ClaArray_airfoil(findAStarACl);

    findClosetValue = min(abs(ClaArray_airfoil_flaps - clatAStarA));

    closetValue = find(abs(ClaArray_airfoil_flaps - clatAStarA) == findClosetValue);

    % Array for end of linear portion of Cla curve
    % aStarPointAirfoilFlaps = [alphaArray_airfoil_flaps(end) ClaArray_airfoil_flaps(end)];
    aStarPointAirfoilFlaps = [alphaArray_airfoil_flaps(closetValue) ClaArray_airfoil_flaps(closetValue)];

    % Delta Clmax for airfoil with flaps
    deltaClmaxAirfoilFlaps = testValue;

    % Array for point of max Cl
    maxPointAirfoilFlaps = [alphaClmax_airfoil_flaps Clmax_airfoil + deltaClmaxAirfoilFlaps];

    % First section (leading) increment array along x-axis 
    leadingIncrementAirfoilFlaps = aStarPointAirfoilFlaps(1):.2:maxPointAirfoilFlaps(1);

    % Leading nonlinear curve
    leadingNonlinearCurveAirfoilFlaps = makima([aStarPointAirfoilFlaps(1)...
        maxPointAirfoilFlaps(1)],[aStarPointAirfoilFlaps(2) maxPointAirfoilFlaps(2)],leadingIncrementAirfoilFlaps);

    % Slope of leading nonlinear curve (LNC)
    slopeLNCAirfoilFlaps = (maxPointAirfoilFlaps(2) - aStarPointAirfoilFlaps(2))...
        / (maxPointAirfoilFlaps(1) - aStarPointAirfoilFlaps(1));

    % Final x-axis point on the last section (trailing) of the nonlinear curve
    stallPointXAirfoilFlaps = ((maxPointAirfoilFlaps(2)-aStarPointAirfoilFlaps(2))...
        / (-slopeLNCAirfoilFlaps)) + leadingIncrementAirfoilFlaps(ceil(length(leadingNonlinearCurveAirfoilFlaps)/1.4)) + alphaClmax_airfoil_flaps;

    % Final y-axis point on the last section (trailing) of the nonlinear curve
    stallPointYFlaps = leadingNonlinearCurveAirfoilFlaps(ceil(length(leadingNonlinearCurveAirfoilFlaps)/1.4));

    % Array of final points
    stallPointAirfoilFlaps  = [stallPointXAirfoilFlaps stallPointYFlaps];

    % Last section (trailing) increment array along x-axis
    % trailingIncrement = maxPointAirfoilFlaps(1):.1:stallPointAirfoilFlaps(1);

    % Array of nonlinear curve points
    nonlinearCurveArrayXAirfoilFlaps = [aStarPointAirfoilFlaps(1) maxPointAirfoilFlaps(1) stallPointAirfoilFlaps(1)];
    nonlinearCurveArrayYAirfoilFlaps = [aStarPointAirfoilFlaps(2) maxPointAirfoilFlaps(2) stallPointAirfoilFlaps(2)];
    nonlinearCurveIncrementAirfoilFlaps = nonlinearCurveArrayXAirfoilFlaps(1):.1:nonlinearCurveArrayXAirfoilFlaps(3);

    % Nonlinear curve of CLa plot
    nonlinearCurveAirfoilFlaps = makima(nonlinearCurveArrayXAirfoilFlaps, nonlinearCurveArrayYAirfoilFlaps,nonlinearCurveIncrementAirfoilFlaps);

    % Need to perform an iteration to determine the optimal value for
    % deltaClmaxAirfoilFlaps
    subArrayIncrement = nonlinearCurveIncrementAirfoilFlaps(1:5);
    subArrayCurve     = nonlinearCurveAirfoilFlaps(1:5);

    % Slope of beginning portion along nonlinear curve
    slopeNonlinear = (subArrayCurve(end)-subArrayCurve(1))/(subArrayIncrement(end)-subArrayIncrement(1));

    % Need to compare nonlinear slope to linear slope
    slopeLinear = (ClaArray_airfoil_flaps(end) - ClaArray_airfoil_flaps(1))/(alphaArray_airfoil_flaps(end) - alphaArray_airfoil_flaps(1));

    difference = (slopeNonlinear - slopeLinear);
    if abs(difference) < .001
        break
    elseif difference < 0
        testValue = testValue + .01;
    elseif difference > 0
        testValue = testValue - .01;
    end
end

% Airfoil Curve without flaps
line(ax,alphaArray_airfoil_flaps(1:closetValue),ClaArray_airfoil_flaps(1:closetValue),'linewidth',2,'color','red');

% Airfoil Curve with flaps
% alphaArray_airfoil_linear = [aL_0_wing; aStarA_wing;];
% ClaArray_airfoil_linear   = [0; CLa_wing*(aStarA_wing-aL_0_wing);];
% line(ax,alphaArray_airfoil_linear_Flaps,ClaArray_airfoil_linear_Flaps,'linewidth',2,'color','black');
line(ax,nonlinearCurveIncrementAirfoilFlaps,nonlinearCurveAirfoilFlaps,'linewidth',2,'color','black');
...
% findAStarACL = find(alphaArray_airfoil == aStarA_wing);
%     
% clatAStarA   = ClaArray_airfoil(findAStarACl);
% 
% findClosetValue = min(abs(ClaArray_airfoil_flaps - clatAStarA));
...
findMaxPointWing = find(nonlinearCurve == max(nonlinearCurve));

maxPointAlphaWing = nonlinearCurveIncrement(findMaxPointWing);

% Array for end of linear portion of Cla curve
aStarPointWingFlaps = [aStarA_wing_Flaps CLa_wing*(aStarA_wing_Flaps - aL_0_wing)];

% Delta CLmax for wing with flaps
deltaCLmaxWingFlaps = deltaClmaxAirfoilFlaps*(SWf/SrefWing)*Kdelta;

% Array for top of Cla curve
% maxPointWingFlaps = [alphaClmax_airfoil_flaps CLmax_wing + deltaCLmaxWingFlaps];
maxPointWingFlaps = [maxPointAlphaWing + deltaAlphaZeroLift_deg CLmax_wing + deltaCLmaxWingFlaps];

% First section (leading) increment array along x-axis 
leadingIncrementWingFlaps = aStarPointWingFlaps(1):.2:maxPointWingFlaps(1);

% Leading nonlinear curve
leadingNonlinearCurveWingFlaps = makima([aStarPointWingFlaps(1) maxPointWingFlaps(1)],[aStarPointWingFlaps(2) maxPointWingFlaps(2)],leadingIncrementWingFlaps);

% Slope of leading nonlinear curve (LNC)
slopeLNCWingFlaps = (maxPointWingFlaps(2) - aStarPointWingFlaps(2)) / (maxPointWingFlaps(1) - aStarPointWingFlaps(1));

% Final x-axis point on the last section (trailing) of the nonlinear curve
stallPointXWingFlaps = ((maxPointWingFlaps(2)-aStarPointWingFlaps(2)) / (-slopeLNCWingFlaps)) + leadingIncrementWingFlaps(ceil(length(leadingNonlinearCurveWingFlaps)/1)) + alphaClmax_airfoil_flaps;

% Final y-axis point on the last section (trailing) of the nonlinear curve
stallPointYWingFlaps = leadingNonlinearCurveWingFlaps(ceil(length(leadingNonlinearCurveWingFlaps)/1));

% Array of final points
stallPointWingFlaps  = [stallPointXWingFlaps stallPointYWingFlaps];

% Last section (trailing) increment array along x-axis
% trailingIncrement = maxPointWingFlaps(1):.1:stallPointWingFlaps(1);

m = (ClaArray_wing_linear_Flaps(end) - ClaArray_wing_linear_Flaps(1)) / (alphaArray_wing_linear_Flaps(end) - alphaArray_wing_linear_Flaps(1));
x2 = (.9160 - 0) / m + alphaArray_wing_linear_Flaps(1);

% Array of nonlinear curve points
% nonlinearCurveArrayXWingFlaps = [aStarPointWingFlaps(1) maxPointWingFlaps(1) stallPointWingFlaps(1)];
nonlinearCurveArrayXWingFlaps = [x2 maxPointWingFlaps(1) stallPointWingFlaps(1)];
nonlinearCurveArrayYWingFlaps = [aStarPointWingFlaps(2) maxPointWingFlaps(2) stallPointWingFlaps(2)];
nonlinearCurveIncrementWingFlaps = nonlinearCurveArrayXWingFlaps(1):.1:nonlinearCurveArrayXWingFlaps(3);

% Nonlinear curve of CLa plot
nonlinearCurve = makima(nonlinearCurveArrayXWingFlaps, nonlinearCurveArrayYWingFlaps,nonlinearCurveIncrementWingFlaps);



% Wing Curve
% alphaArray_wing_linear_Flaps = [aL_0_wing_Flaps; aStarA_wing;];
alphaArray_wing_linear_Flaps = [aL_0_wing_Flaps; x2];
% ClaArray_wing_linear_Flaps   = [0; CLa_wing*(aStarA_wing-aL_0_wing_Flaps);];
% ClaArray_wing_linear_Flaps   = [0; CLa_wing*(aStarA_wing-aL_0_wing_Flaps);];
ClaArray_wing_linear_Flaps   = [0; nonlinearCurve(1)];





line(ax,alphaArray_wing_linear_Flaps,ClaArray_wing_linear_Flaps,'linewidth',2,'color','black');
line(ax,nonlinearCurveIncrementWingFlaps,nonlinearCurve,'linewidth',2,'color','black');
...
%         end
    ...
    % Plots 3D wing curve with/without flaps (10/25/2021)
