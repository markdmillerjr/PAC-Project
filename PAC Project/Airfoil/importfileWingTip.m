function [NACA,VarName2] = importfileWingTip(filename, startRow, endRow)
...
%    See also TEXTSCAN.
ii1 = [];
global NACA VarName2 wingAirfoilFileTip fileID masterFile
% file = 'm1.dat';
...
% Access Airfoil Database folder
cd('Airfoil Database');

% Find specified airfoil in Airfoil Database
wingAirfoilFileDir = dir(wingAirfoilFileTip);

% Input specified airfoil into MATLAB
wingAirfoilFileTip = wingAirfoilFileDir.name;

%% Open the text file.
fileID = fopen(wingAirfoilFileTip,'r');
...
% Code to fix airfoil array import from textfile
    ...
    if NACA(2) == NACA(3)
     ...
     if isinteger(VarName2(1)) ~= 1
         ...
         VarName2 = VarName2(3:end);
     end

     for ii1 = 1:(length(VarName2)-1)
         if VarName2(ii1) ~= 0
             if isnan(VarName2(ii1))
                 VarName2(ii1) = 0;
             end
         end
     end
...
%       for ii5 = 1:(length(VarName2)-2)
%               if VarName2(1) == 0
%                   VarName2(1) = [];
%               elseif VarName2(end) == 0;
%                   VarName2(end) = [];
%               end
%       end      
     end
